...
{
    CachedSegment * s;
    s = (CachedSegment * )av_mallocz(sizeof(CachedSegment));
    if(s == NULL){
        return NULL;
    }
...
{  
    ...
    if(segment->head == NULL){
        segment->head = segment->tail = (fragment *)av_malloc(FRAGMENT_SIZE);
        if(segment->head == NULL){
            //error log
            ret = -1;
            return ret;
    ...
    while(need_write){
        ...
        if(left_space == 0){
            ...
            segment->tail->next = (fragment *)av_malloc(FRAGMENT_SIZE);
