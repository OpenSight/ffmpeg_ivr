...
fail:    
    if(ret < 0){
        av_log(NULL, AV_LOG_ERROR,  "[cseg_ivt_writer] HTTP POST failed:%s\n", err_buf);        
    }
    if(headers != NULL){
...
fail:    
    ...
    if(ret < 0){
        av_log(NULL, AV_LOG_ERROR,  "[cseg_ivt_writer] HTTP PUT failed:%s\n", err_buf);        
    }
                       ...
                       char * file_uri, int file_uri_size)
{
    ...
    if(status_code >= 200 && status_code < 300){
        ...
        if(json_root== NULL){
            ret = AVERROR(EINVAL);
            av_log(NULL, AV_LOG_ERROR,  "[cseg_ivt_writer] HTTP response Json parse failed(%s)\n", http_response_json);
            goto failed;
        }
        ...
        if(response_size != 0){
            json_root = cJSON_Parse(http_response_json);
            if(json_root== NULL){
                av_log(NULL, AV_LOG_ERROR, "[cseg_ivt_writer] HTTP create file (%s) status code(%d):%s\n", 
                ...
                if(json_info && json_info->type == cJSON_String && json_info->valuestring){            
                    av_log(NULL, AV_LOG_ERROR,  "[cseg_ivt_writer] HTTP create file status code(%d):%s\n", 
...
static int write_file(IvrWriterPriv * priv,
                       CachedSegment *segment, 
                       int32_t io_timeout, 
                       char * file_path)
{
    ...
    ret = avio_open(&file_context, file_path, AVIO_FLAG_WRITE);
    if(ret < 0){
        av_log(NULL, AV_LOG_ERROR,  "[cseg_ivt_writer] open file failed, avio_open() failed with ret(%d)\n", 
                   ret);  
        return ret;
    }
    ...
    if(ret < 0){
        ...
        av_log(NULL, AV_LOG_ERROR,  "[cseg_ivt_writer] write file failed, avio_write() failed with ret(%d)\n", 
                   ret); 
        return ret;
    }
    avio_closep(&file_context);    
                      ...
                      int success)
{
    ...
    if(status_code < 200 || status_code >= 300){
        ...
        if(response_size != 0){
            json_root = cJSON_Parse(http_response_json);
            if(json_root== NULL){
                av_log(NULL, AV_LOG_ERROR,  "[cseg_ivt_writer] HTTP save file (%s) status code(%d):%s\n", 
                       priv->ivr_rest_uri, status_code, http_response_json);                           
            }else{
                json_info = cJSON_GetObjectItem(json_root, IVR_ERR_INFO_FIELD_KEY);
                if(json_info && json_info->type == cJSON_String && json_info->valuestring){
...
static int ivt_init(CachedSegmentContext *cseg)
{
    int ret = 0; 
    char *p;
    ...
    if(cseg->filename == NULL || strlen(cseg->filename) == 0){
        ret = AVERROR(EINVAL);
        av_log(NULL, AV_LOG_ERROR, "[cseg_ivt_writer] http filename absent\n");          
        goto fail;       
    }
    
    if(strlen(cseg->filename) > (MAX_URI_LEN - 5)){
        ret = AVERROR(EINVAL);
        av_log(NULL, AV_LOG_ERROR,  "[cseg_ivt_writer] filename is too long\n");          
        goto fail;
    }
    ...
    if(p){
        ...
        ret = AVERROR(EINVAL);
        av_log(NULL, AV_LOG_ERROR,  "[cseg_ivt_writer] filename malformat\n");
        goto fail;
    }  
...
#define FILE_CREATE_TIMEOUT 5000
static int ivt_write_segment(CachedSegmentContext *cseg, CachedSegment *segment)
{
    IvrWriterPriv * priv = (IvrWriterPriv * )cseg->writer_priv;   
    char file_path[MAX_URI_LEN];
    ...
    if(strlen(filename) == 0 || strlen(file_path) == 0){
        ret = 1; //cannot write file at the moment
        
    }else{    
        
        //upload segment to the file URI
        ret = write_file(priv, segment, 
                          cseg->writer_timeout,
                          file_path);                      
        if(ret == 0){
...
static void ivt_uninit(CachedSegmentContext *cseg)
{
...
CachedSegmentWriter cseg_ivt_writer = {
    .name           = "ivt_writer",
    .long_name      = "IVT local File system segment writer", 
    .protos         = "ivt", 
    .init           = ivt_init, 
    .write_segment  = ivt_write_segment, 
    .uninit         = ivt_uninit,
};
